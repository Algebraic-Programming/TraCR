...
INSTR_0ARG(instr_thread_enter_warming, "OHw")
INSTR_0ARG(instr_thread_enter_cooling, "OHc")
INSTR_0ARG(instr_event_enter, "OU[")
INSTR_0ARG(instr_event_exit, "OU]")
...
/* simplified proc initialization */
static inline void
instr_init_proc()
{
...
/* simplified thread initialization */
static inline void
instr_init_thread(int rank)
{
...
instr_start(int rank, int nranks)
{
	...
	dbg("thread %d has pid %d and cpu %d (ncpus=%d)\n",
			get_tid(), getpid(), curcpu, nranks);
